...
(defun emit-mouse-move (ui layer x y &optional other)
  (when layer (setf (layer-dirty layer) t))
  (setf (ui-cursor-layer ui) layer)
  (setf (ui-cursor-x ui) x)
  (setf (ui-cursor-y ui) y)
  (let ((move-handled nil)
        (drag-handled nil)
        (prev-drag-owner (ui-drag-owner ui)))
    (traverse-ui-with-mouse ui layer x y
                            (lambda (widget)
                              (if (hit-widget-p ui widget x y)
                                  (progn
                                   (when (and (widget-on-mouse-move widget)
                                              (null (ui-mouse-owner ui))
                                              (not move-handled))
                                         (setf move-handled (funcall (widget-on-mouse-move widget) ui widget x y other)))
                                   (unless (funcall (widget-get-mouse-hover widget))
                                     (funcall (widget-set-mouse-hover widget) t)
                                     (when (widget-on-mouse-enter widget)
                                           (funcall (widget-on-mouse-enter widget) ui widget x y other)))
                                   (when (and (ui-drag-value ui) (not drag-handled))
                                         (if (eq prev-drag-owner widget)
                                             (when (widget-on-drag-move prev-drag-owner)
                                                   (setf drag-handled (funcall (widget-on-drag-move prev-drag-owner) ui widget x y)))
                                             (when (widget-on-drag-enter widget)
                                                   (setf drag-handled (funcall (widget-on-drag-enter widget) ui widget x y))
                                                   (when drag-handled
                                                         (when (and prev-drag-owner (widget-on-drag-leave prev-drag-owner))
                                                               (funcall (widget-on-drag-leave prev-drag-owner) ui prev-drag-owner))
                                                         (setf (ui-drag-owner ui) widget)))))
                                   nil)
                                  (progn
                                   (when (funcall (widget-get-mouse-hover widget))
                                         (funcall (widget-set-mouse-hover widget) nil)
                                         (when (widget-on-mouse-leave widget)
                                               (funcall (widget-on-mouse-leave widget) ui widget x y other)))
                                   nil))))
    (let ((mouse-owner (ui-mouse-owner ui)))
      (and mouse-owner
           (widget-on-mouse-move mouse-owner)
           (funcall (widget-on-mouse-move mouse-owner) ui mouse-owner x y other)))
    (when (and (not drag-handled) prev-drag-owner) ; Not handled => current owner = previous
          (when (widget-on-drag-leave prev-drag-owner)
                (funcall (widget-on-drag-leave prev-drag-owner) ui prev-drag-owner))
...
(defun emit-mouse-scroll (ui delta-x delta-y &optional other)
  (when (ui-cursor-layer ui)
        (setf (layer-dirty (ui-cursor-layer ui)) t))
      ...
      (defun ,fname (ui layer x y &optional other)
        ...
        ,@(when (eq typ :down)
                `((when (ui-reset-focus-on-mouse-down ui)
                  ...
                  (let ((focused-windows (or (gethash layer (ui-focused-windows ui)) (make-array 0 :adjustable t :fill-pointer 0))))
                    (loop #:for index #:below (length focused-windows)
                          #:do (setf (aref focused-windows index) nil))
                    (setf (fill-pointer focused-windows) 0)
                    (multiple-value-bind (windows present)
                        (gethash layer (ui-windows ui))
                      (when present
                            (loop #:for index #:from (1- (length windows)) #:downto 0
            ...
            (let ((widget (ui-mouse-owner ui)))
              ...
              ,(ecase typ
                 (:down `(when (hit-widget-p ui widget x y)
                               (funcall (widget-set-mouse-active widget)
                                 (logior ,active-bit-name (funcall (widget-get-mouse-active widget))))))
                 (:up `(let ((widget-active-mouse-buttons (funcall (widget-get-mouse-active widget))))
                                                    ...
                                                    (prog1
                                                      ...
                                                      ,(when (eq typ :down)
                                                             `(funcall (widget-set-mouse-active widget)
                                                                (logior ,active-bit-name (funcall (widget-get-mouse-active widget)))))))
                                            ,(when (eq typ :up)
                                                   `(let ((widget-active-mouse-buttons (funcall (widget-get-mouse-active widget))))
                                                      (when (< 0 (logand ,active-bit-name widget-active-mouse-buttons))
                                                            (funcall (widget-set-mouse-active widget)
                                                              (logxor ,active-bit-name widget-active-mouse-buttons))
