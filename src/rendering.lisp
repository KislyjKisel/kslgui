...
(defun render-window (ui window clip-rect translate-point)
  ...
  (compute-widget-coordinates ui (window-widget window)
                              0.0 0.0
                              (coerce (window-x window) 'single-float)
                              (coerce (window-y window) 'single-float))
...
(export 'render)
(declaim (ftype (function (ui double-float &key (:clear boolean) (:defer-flush boolean)) (values &optional)) render))
(defun render (ui time &key (clear t) (defer-flush nil))
  (when (sdet:run-effects (ui-sdet-context ui))
        (alexandria:maphash-keys (lambda (layer) (setf (layer-dirty layer) t)) (ui-windows ui)))
  (when (null (ui-start-time ui))
        (setf (ui-start-time ui) time)
        (setf (ui-previous-frame-time ui) time))
  (setf (ui-time ui) (- time (ui-start-time ui)))
  (setf (ui-delta-time ui) (- time (ui-previous-frame-time ui)))
  (setf (ui-previous-frame-time ui) time)
