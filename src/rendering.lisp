...
(declaim (ftype (function (ui window %blend2d:rect %blend2d:point) (values &optional)) render-window-impl))
(defun render-window-impl (ui window clip-rect translate-point)
  (when (not (window-widget window))
        (when (window-resize-to-contents-x window)
              (setf (window-width window) 0.0d0))
        (when (window-resize-to-contents-y window)
              (setf (window-height window) 0.0d0))
        (return-from render-window-impl (values)))
  ...
  (when (window-resize-to-contents-x window)
        (setf (window-width window)
          (coerce (yogalayout:node-layout-get-width (widget-yoga-node (window-widget window)))
                  'double-float)))
  (when (window-resize-to-contents-y window)
        (setf (window-height window)
          (coerce (yogalayout:node-layout-get-height (widget-yoga-node (window-widget window)))
                  'double-float)))
...
(declaim (ftype (function (ui window) (values &optional)) render-window))
(defun render-window (ui window)
